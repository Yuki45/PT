...
namespace repairhhp.Controls
{
    public class Attractive
    {
        ...
        public SqlDataAdapter getSql(string sql1)
        {
            SqlConnection connection = _Conn.Connect();
        ...
        public string UpProcAtt(string _model_name_act, string _modelAtt, string _OriginLine, string AttLine, string process,string AttRemark)
        {
            DataTable hasil = new DataTable();
            string time = "";
            SqlConnection connection = _Conn.Connect();
            try
            {
                ...
                string sql = string.Format(@"INSERT INTO [MTC].[dbo].[tbl_att_line]
                                            ([model_actual]
                                            ,[model_att]
                                            ,[origin_line]
                                            ,[att_line]
                                            ,[process_att]
                                            ,[Att_Remark])
                 VALUES  ('" + _model_name_act + "' ,'" + _modelAtt + "','" + _OriginLine + "' ,'" + AttLine + "','" + process + "', '"+AttRemark+"')");
                connection.Open();   
                command = new SqlCommand(sql, connection);
                    int numberOfRecords = command.ExecuteNonQuery();

...
namespace repairhhp.Controls
{
    public class Machine
    {
        ...
        public DataTable getTable(string cline,string start, string end, string timestart, string timeend)
        {
            ...
            string datec = " And d.[INSPECT TIME] between '" + start + " " + timestart + ":00:00.000' And '" + end + " " + timeend + ":59:59.000' ";//"Where Cast([WRITE DATE] as date) = Cast('" + dtwrite.Value.ToString("yyyy-MM-dd") + "' as date) ";
            string date = " Where h.[INSPECT TIME] between '" + start + " " + timestart + ":00:00.000' And '" + end + " " + timeend + ":59:59.000' ";//"Where Cast([WRITE DATE] as date) = Cast('" + dtwrite.Value.ToString("yyyy-MM-dd") + "' as date) ";
